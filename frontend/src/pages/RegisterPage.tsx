...
export function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.data?.message) {
        ...
        setServerError('Не удалось зарегистрироваться. Проверьте подключение и попробуйте снова.');
      }
    }
  };
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '12px' }}>
              <p style={{ margin: '0 0 5px', fontSize: '14px', color: '#1d849f', userSelect: 'none' }}>ФИО:</p>
              <input type="text" placeholder="Иванова Мария Васильевна" style={mobileInputStyle} {...register('fullName')} />
              {errors.fullName && <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#e53e3e' }}>{errors.fullName.message}</p>}
            </div>
            ...
            <div style={{ marginBottom: '18px' }}>
              <p style={{ margin: '0 0 5px', fontSize: '14px', color: '#1d849f', userSelect: 'none' }}>Придумайте пароль:</p>
              <input type="password" autoComplete="new-password" placeholder="Минимум 8 символов" style={mobileInputStyle} {...register('password')} />
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              ...
              >
                <span style={{ fontWeight: 700, fontSize: '15px', color: '#1d5a7a' }}>
                  {isSubmitting ? 'Создаём кабинет...' : 'Зарегистрироваться'}
                </span>
              </div>
            </div>

            <Link to="/login" style={{ display: 'block', margin: '12px 0 0', fontSize: '13px', color: '#1d849f', textAlign: 'center', textDecoration: 'none', userSelect: 'none' }}>
              Уже есть аккаунт? <span style={{ color: '#21a038' }}>Войти.</span>
            </Link>
          </div>
        </div>
  ...
  const iW = cardW - 110;
  const iL = cardL + 55;
  const dy = { fio: 112, email: 252, pass: 392, btn: 522, link: 614 };
  const btnW = 360;
  ...
  return (
    <div style={{ width: '100vw', height: `${H * scale}px`, overflow: 'hidden', background: '#fff' }}>
      <div style={{
        ...
        {/* ФИО */}
        <p style={{
          position: 'absolute', left: `${iL}px`, top: `${cardT + dy.fio}px`,
        ...
        <input
          type="text"
          placeholder="Иванова Мария Васильевна"
          style={fieldStyle(iL, cardT + dy.fio + 32, iW)}
          {...register('fullName')}
        />
        {errors.fullName && (
          <p style={{ position: 'absolute', left: `${iL}px`, top: `${cardT + dy.fio + 98}px`, margin: 0, fontFamily: LP, fontSize: '18px', color: '#e53e3e' }}>
        ...
        <div
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#fff'; }}
        >
          <span style={{ fontFamily: LP, fontWeight: 700, fontSize: '26px', color: '#1d5a7a' }}>
            {isSubmitting ? 'Создаём кабинет...' : 'Зарегистрироваться'}
          </span>
        </div>
